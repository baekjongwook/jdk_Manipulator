...
Node::Node(int &argc, char **argv) : test(&s, &serialRead, &m, &readByte), manipulator(&s, &serialRead, &m, &readByte, &initPub), minibot(&s, &serialRead, &m, &readByte)
{
  // ros init
  ros::init(argc, argv, "jdk");
  ros::start();
  ros::NodeHandle n;

  // manipulator topics
  //initPub = n.advertise<std_msgs::Bool>("/init_ok", 100);
  //ikSub = n.subscribe("/control_mani", 1, &Manipulator::ikCallback, &manipulator);
  //gripperSub = n.subscribe("/control_gripper", 1, &Manipulator::gripperCallback, &manipulator);

  // minibot topics
  joySub = n.subscribe("/joy", 10, &Minibot::joyCallback, &minibot);
...
void Node::timerCallback()
{
  ...
  test.Algorithm_Test();
  //manipulator.Algorithm_Manipulator();
  //minibot.Algorithm_Minibot();
}

void Node::serialReadThread()
{
  // 수신한 바이트를 저장할 버퍼
  unsigned char readBuffer[1000];
